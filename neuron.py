...
class Neuron:
    def __init__(self, outputs_number, index):
        ...
        for output_index in range(outputs_number):
            self.connections.append({"weight": self.random_weight(), "delta": 0.0,
                                     "sum_of_delta": 0.1})  # ustawienie wag losowo [0-1], poprzednia delta - momentum, alpha.t - adaptative
    ...
    def feed_forward(self, prev_layer, prev_layer_index):
        summed_values = 0.0
    ...
    def update_neuron_weights(self, layer_on_the_left):
        ...
        for i in range(len(layer_on_the_left)):
            ...
            old_delta_weight = neuron.connections[self.index]["delta"]  # poprzednia zmiana wagi pomiedzy rozwazanym neuronem a i-tym neuronem z warstwy poprzedniej
            lr = self.learning_rate / (self.epsilon + neuron.connections[self.index]["sum_of_delta"]) ** 0.5
            #print("Currnet learning rate: {}".format(lr))
            new_delta_weight = neuron.output_value * self.gradient * lr + self.momentum_rate * old_delta_weight
            neuron.connections[self.index]["weight"] -= new_delta_weight
            neuron.connections[self.index]["delta"] = new_delta_weight
            neuron.connections[self.index]["sum_of_delta"] += new_delta_weight ** 2
